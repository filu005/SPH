...
void Emitters::emit()
{
	for (auto & emitter : _emitters)
	{
		...
		if (emitter.last_emission_time >= emitter.delay)
		{
			...
			particle_system_ref->add_particle(Particle(new_position, emitter.emit_velocity, emitter.type, proliferative, 0, 200, 1, emitter.nutrient, emitter.density, emitter.fluid_rest_density, emitter.fluid_viscosity, emitter.particle_mass, emitter.color_value));
